...
/** Standard OpenAI-compatible /v1/models parser */
const openaiStyleParser = (json: unknown) => {
...
const PROVIDER_CONFIGS: Partial<Record<Provider, ProviderConfig>> = {
	openai: {
		listModelsUrl: 'https://api.openai.com/v1/models',
		authHeader: bearerAuth,
		parseModels: openaiStyleParser,
		filterModel: (id) =>
	...
	fireworks: {
		listModelsUrl: 'https://api.fireworks.ai/inference/v1/models',
		authHeader: bearerAuth,
		parseModels: openaiStyleParser,
	},
	togetherai: {
	...
	cohere: {
		listModelsUrl: 'https://api.cohere.ai/v1/models',
		authHeader: bearerAuth,
		parseModels: (json) => {
			const models = (json as { models: { name: string }[] }).models
	...
	cerebras: {
		listModelsUrl: 'https://api.cerebras.ai/v1/models',
		authHeader: bearerAuth,
		parseModels: openaiStyleParser,
	},
	deepinfra: {
		listModelsUrl: 'https://api.deepinfra.com/v1/openai/models',
		authHeader: bearerAuth,
		parseModels: openaiStyleParser,
	},
	moonshotai: {
	...
	// alibaba, huggingface, baseten, amazon-bedrock, google-vertex, openai-compatible
	// intentionally omitted — no standard model list endpoints
}

function isProvider(name: string): name is Provider {
	return ProviderSchema.options.includes(name as Provider)
}

async function fetchModelsRaw(provider: Provider, apiKey: string): Promise<string[]> {
	const config = PROVIDER_CONFIGS[provider]
	if (!config) {
		throw new Error(`No model listing config for provider: ${provider}`)
	}
	...
	// Providers with dynamic URLs (e.g. Azure needs resource name, Ollama needs baseURL)
	if (config.buildUrl) {
...
export async function validateApiKey(provider: string): Promise<boolean> {
	...
	const config = PROVIDER_CONFIGS[provider]
	if (!config) return false // No model listing available for this provider

	// Ollama doesn't use API keys
	if (provider === 'ollama') {
		try {
			await fetchModelsRaw(provider, '')
			return true
		} catch {
			return false
		}
	...
	// Providers with buildUrl (Azure, Ollama) — can't validate without config
	if (config.buildUrl) {
		const providerCfg = getProviderConfig(provider)
...
export async function listModels(provider: string): Promise<string[]> {
	...
	// Provider has no model listing config — return empty
